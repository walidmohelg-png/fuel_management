@php
    // تسهيل الوصول للبيانات (يجب أن يكون موجوداً إذا نجح الحفظ)
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    ...
    <div>
        ...
        <div class="mt-4">
            ...
            <select id="distributor_id" name="distributor_id" required class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                ...
                @foreach ($distributors as $distributor)
                    {{-- عرض القيمة المحفوظة --}}
                    <option value="{{ $distributor->id }}" {{ ($company->distributor_id == $distributor->id || old('distributor_id') == $distributor->id) ? 'selected' : '' }}>
                        {{ $distributor->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mt-4">
            ...
            <x-text-input id="activity_type" class="block mt-1 w-full" type="text" name="activity_type" :value="old('activity_type', $company->activity_type)" />
        </div>
        
        {{-- 4. الرمز (مثل NS-001) --}}
        <div class="mt-4">
            <x-input-label for="fuel_code" :value="__('رمز الشركة')" />
            <x-text-input id="fuel_code" class="block mt-1 w-full" type="text" name="fuel_code" :value="old('fuel_code', $company->fuel_code)" />
        </div>
        ...
        <div class="mt-4">
            ...
            <select id="current_status" name="current_status" required class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                <option value="pending" {{ old('current_status', $company->current_status) == 'pending' ? 'selected' : '' }}>{{ __('تحت الإجراء') }}</option>
                <option value="active" {{ old('current_status', $company->current_status) == 'active' ? 'selected' : '' }}>{{ __('مفعلة') }}</option>
                <option value="suspended" {{ old('current_status', $company->current_status) == 'suspended' ? 'selected' : '' }}>{{ __('موقوفة') }}</option>
                <option value="expired" {{ old('current_status', $company->current_status) == 'expired' ? 'selected' : '' }}>{{ __('انتهاء الصلاحية') }}</option>
            </select>
        </div>
    </div>
    ...
    <div>
        ...
        <div class="mt-4">
            ...
            <select id="fuel_type" name="fuel_type" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                <option value="ديزل" {{ old('fuel_type', $details->fuel_type ?? '') == 'ديزل' ? 'selected' : '' }}>{{ __('ديزل') }}</option>
                <option value="بنزين" {{ old('fuel_type', $details->fuel_type ?? '') == 'بنزين' ? 'selected' : '' }}>{{ __('بنزين') }}</option>
            </select>
        </div>
        ...
        <div class="mt-4">
            ...
            <x-text-input id="monthly_allowance" class="block mt-1 w-full" type="number" name="monthly_allowance" :value="old('monthly_allowance', $details->monthly_allowance ?? '')" />
        </div>

        {{-- 8. تاريخ بدء سريان المخصص --}}
        <div class="mt-4">
            <x-input-label for="effective_date" :value="__('تاريخ بدء سريان المخصص')" />
            <x-text-input id="effective_date" class="block mt-1 w-full" type="date" name="effective_date" :value="old('effective_date', $details->effective_date ?? '')" />
        </div>

        {{-- 9. مستودع التزويد --}}
        <div class="mt-4">
            <x-input-label for="supply_warehouse" :value="__('مستودع التزويد (نص)')" />
            <x-text-input id="supply_warehouse" class="block mt-1 w-full" type="text" name="supply_warehouse" :value="old('supply_warehouse', $details->supply_warehouse ?? '')" />
        </div>
        ...
        <div class="mt-4">
            ...
            <x-text-input id="nearest_landmark" class="block mt-1 w-full" type="text" name="nearest_landmark" :value="old('nearest_landmark', $company->nearest_landmark)" />
        </div>
        ...
        <div class="flex space-x-4 space-x-reverse">
            ...
            <div class="mt-4 w-1/2">
                ...
                <x-text-input id="latitude" class="block mt-1 w-full" type="text" name="latitude" :value="old('latitude', $company->latitude)" />
            </div>

            {{-- خط الطول (Longitude) --}}
            <div class="mt-4 w-1/2">
                <x-input-label for="longitude" :value="__('خط الطول (Longitude)')" />
                <x-text-input id="longitude" class="block mt-1 w-full" type="text" name="longitude" :value="old('longitude', $company->longitude)" />
            </div>
        </div>
